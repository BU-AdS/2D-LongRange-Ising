...
term1s(x) = A*0.5*exp(-D*L*scale)*cosh(D*(x-L)*scale) 
term1c(x) = B*0.5*exp(-(2-D)*L*scale)*cosh((2-D)*(x-L)*scale)
term2s(x) = A*D*COS_T*exp(-(D+a)*L*scale)*cosh((D+a)*(x-L)*scale)
term2c(x) = B*(2-D)*COS_T*exp(-((2-D)+a)*L*scale)*cosh(((2-D)+a)*(x-L)*scale)
term3s(x) = A*D*(2*D+1)*COS_T*COS_T*exp(-(D+b)*L*scale)*cosh((D+b)*(x-L)*scale)
term3c(x) = B*(2-D)*(2*(2-D)+1)*COS_T*COS_T*exp(-((2-D)+b)*L*scale)*cosh(((2-D)+b)*(x-L)*scale)
...
fit [1:L] term1s(x) + term1c(x) + term2s(x) + term2c(x) + term3s(x) + term3c(x) + term4s(x) + term4c(x) + term5(x) + term6(x) 'correlators.dat' using 1:2:3 via A,scale,B,D

set label sprintf("Coefficients\nC_{0} = %e\nC_{1} = %e\nC_{2} = %e\nC_{3} = %e\n\nC_{/Symbol c}/C_{/Symbol s} = %f\n\n{/Symbol D} = %f\n\na = %f", 0.5*exp(-D*L*scale), D*COS_T*exp(-(D+a)*L*scale), D*(2*D+1)*COS_T*COS_T*exp(-(D+b)*L*scale), (2.0/3.0)*COS_T*D*(D+1)*(2*COS_T*COS_T*(D+2)-3)*exp(-(D+c)*L*scale), B/A, D, scale) at 2.5,0.38
