...
#else
  cout<<"Couplings created on the fly"<<endl;
#endif
...
#pragma omp for nowait 
	...
	for(int k=0; k<chunk; k++) { 
	  ...
	  if( (j+k) != i ) {
	    ...
	    //coefficient.
	    //FIXME	  
	    val = 0.25*(phi - phi_arr[j+k])*(phi - phi_arr[j+k])/sqrt(dx*dx + dt*dt);
...
void createLRcouplings(double *LR_couplings, Param p) {
  ...
  if(p.coupling_type == SR) {
	...
	for(int i=0; i<j; i++) { 
	  ...
	  LR_couplings[j+i*p.surfaceVol] = pow(sqrt(dx*dx + dt*dt),-(2+sigma));
	  LR_couplings[i+j*p.surfaceVol] = LR_couplings[j+i*p.surfaceVol];
	  //Temporal
...
void LRAdSCouplings(double *LR_couplings, Param &p){
  ...
  for(int j=0; j<p.surfaceVol; j++){
    for(int i=0; i<j; i++){
      ...
      LR_couplings[j+i*p.surfaceVol] = pow(couplingNorm*Coupling(dt, dth, p), (2+sigma)/2);
      LR_couplings[i+j*p.surfaceVol] = LR_couplings[j+i*p.surfaceVol];
